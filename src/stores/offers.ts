...
export const useClientStore = defineStore("clients", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`offers/soft-delete/${id}`);
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`offers/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("offers", credentials);
