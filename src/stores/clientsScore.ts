...
export const useClientScoreStore = defineStore("clients-scores", () => {
  const errors = ref({});
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`clients-scores/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`clients-scores`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`clients-scores/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`clients-scores/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("clients-scores", credentials);
