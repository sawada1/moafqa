...
export const useCityStore = defineStore("city", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`cities/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`cities`, "", { page: page })
      .then(({ data }) => {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`cities/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("cities", credentials);
