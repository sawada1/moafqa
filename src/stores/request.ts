...
export const useRequestStore = defineStore("requests", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`requests/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`requests`, "", { page: page })
      .then(({ data }) => {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`requests/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("requests", credentials);
