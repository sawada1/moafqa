...
export const useModelsStore = defineStore("models", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`models/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`models`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`models/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`models/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("models", credentials);
