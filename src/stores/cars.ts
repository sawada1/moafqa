...
export const useCarsStore = defineStore("cars", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`cars/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`cars`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`cars/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any, addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`cars/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(credentials: any, addCustomerModalRef) {
    try {
      const response = await ApiService.post("cars", credentials);
      if (response.status >= 201) {
        errorSubmit.value = true;
        errors.value = {};
        loading.value = false;
        for (const itemm in credentials) {
          credentials[itemm] = Array.isArray(credentials[itemm]) ? [] : "";
        }
        emptyForm.value = credentials;
        setTimeout(() => {
          Swal.fire({
            text: "Form has been successfully submitted!",
            icon: "success",
            class:'carFormSuccess',
            buttonsStyling: false,
            confirmButtonText: "Ok, got it!",
            heightAuto: false,
            customClass: {
          ...
          document.getElementById("kt_modal_add_cars_cancel")?.click();
        }, 500);
      } else {
        console.error("Unexpected status code:", response.status);
      }
