...
export const useColorsStore = defineStore("colors", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`colors/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`colors`, "", { page: page })
      .then(({ data }) => {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`colors/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("colors", credentials);
