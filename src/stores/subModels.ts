...
export const useSubModelsStore = defineStore("subModels", () => {
  const errors = ref({});
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`sub-models/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`sub-models`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`sub-models/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`sub-models/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("sub-models", credentials);
