...
export const useClientStore = defineStore("clients", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`client/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`client`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`client/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`client/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("client", credentials);
