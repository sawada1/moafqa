...
export const useFundsStore = defineStore("funds", () => {
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(`fund_companies/soft-delete/${id}`);
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`fund_companies`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`fund_companies/${id}`);
      if (response.status === 200) {
  ...
  async function edit(id: number, credentials: any , addCustomerModalRef) {
    try {
      const response = await ApiService.patch(`fund_companies/${id}`, credentials);
      if (response.status >= 200) {
  ...
  async function create(
    ...
    try {
      const response = await ApiService.post("fund_companies", credentials);
