...
export const useBrnchAgencyStore = defineStore("branch-agency", () => {
  const errors = ref({});
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(
        `agency-branches/soft-delete/${id}`
      );
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`agency-branches`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`agency-branches/${id}`);
      if (response.status === 200) {
  ...
  async function create(credentials: any, addCustomerModalRef) {
    try {
      const response = await ApiService.post("agency-branches", credentials);
