...
export const useBrnchFundStore = defineStore("fund_company_branches", () => {
  const errors = ref({});
  ...
  async function deleteFunc(id: number) {
    try {
      const response = await ApiService.delete(
        `fund_company_branches/soft-delete/${id}`
      );
  ...
  function getItems(page = 1 as number) {
    return ApiService.get(`fund_company_branches`, "", { page: page })
      .then(({ data }) => {
  ...
  async function getItem(id: any) {
    try {
      const response = await ApiService.get(`fund_company_branches/${id}`);
      if (response.status === 200) {
    ...
    try {
      ...
      if (response.status >= 201) {
        ...
        setTimeout(() => {
          ...
          document.getElementById("kt_modal_add_branch_cancel")?.click();
        }, 1000);
      } else {
        console.error("Unexpected status code:", response.status);
      }
