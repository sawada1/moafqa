  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      <KTDatatable
        ...
        :total="store.total"
      >
        <template v-slot:price="{ row: customer }">
          {{ customer.price }}
        </template>
        <template v-slot:year="{ row: customer }">
          {{ customer.year }}
        </template>
        <template v-slot:car_body="{ row: customer }">
          {{ customer.car_body }}
        </template>
        <template v-slot:colors="{ row: customer }">
          <div v-for="color in customer.colors">
            <span class="">
              {{ color.name_ar }} {{ customer.colors.length > 1 ? " , " : "" }}
            </span>
          </div>
        </template>
        <template v-slot:fuel_type="{ row: customer }">
          {{ customer.fuel_type }}
        </template>
        <template v-slot:gear_shifter="{ row: customer }">
          {{ customer.gear_shifter }}
        </template>
...
import AddModal from "@/components/modals/forms/cars/AddModel.vue";
import EditModal from "@/components/modals/forms/cars/EditModel.vue";
  ...
  setup() {
    const headerConfig = ref([
      ...
      },
      {
        ...
        columnLabel: "fuel_type",
        sortEnabled: true,
      },
      {
        columnName: "gear shifter",
        columnLabel: "gear_shifter",
        sortEnabled: true,
      },
      {
        columnName: "Actions",
