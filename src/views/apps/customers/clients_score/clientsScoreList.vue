  ...
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:commercial_register="{ row: customer }">
          {{ customer.commercial_register }}
        </template>
        <template v-slot:fund_company_type="{ row: customer }">
          {{ customer.fund_company_type }}
        </template>
              ...
              <v-menu activator="parent">
                <div class="custom-menu-container">
                  ...
                  <div class="menu-item px-3 link-menu-custom">
                    ...
                    >
                      <a class="menu-link px-3">{{ $t('edit') }}</a>
                    </button>
                  </div>
                  <!--end::Menu item-->
                </div>
...
import AddModal from "@/components/modals/forms/clientScore/AddModel.vue";
import EditModal from "@/components/modals/forms/clientScore/EditModel.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const headerConfig = ref([
      {
        columnName: locale.value == "en" ? "name" : "الاسم",
        columnLabel: "name",
        sortEnabled: true,
      },
      {
        columnName: locale.value == "en" ? "email" : "البريد الالكتروني",
        columnLabel: "email",
        sortEnabled: true,
      },
      {
        columnName: locale.value == "en" ? "mobile" : "الهاتف",
        columnLabel: "mobile",
        sortEnabled: true,
      },
      {
        columnName:
          locale.value == "en" ? "commercial register" : "السجل التجاري",
        columnLabel: "commercial_register",
        sortEnabled: true,
      },
      {
        columnName:
          locale.value == "en" ? "fund company type" : "نوع شركات التمويل",
        columnLabel: "fund_company_type",
        sortEnabled: true,
      },
      {
        columnName: locale.value == "en" ? "actions" : "الأجراءات",
        columnLabel: "actions",
        sortEnabled: false,
        columnWidth: 135,
      },
