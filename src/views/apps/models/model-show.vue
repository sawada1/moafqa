<template>
  <!--begin::Layout-->
  <div class="d-flex flex-column flex-xl-row">
    <!--begin::Sidebar-->
    <div
      v-if="!pending"
      class="flex-column flex-lg-row-auto w-100 w-xl-350px mb-10"
    >
      <!--begin::Card-->
      <div v-if="brand" class="card mb-5 mb-xl-8">
        <!--begin::Card body-->
        <div class="card-body pt-15">
          <!--begin::Summary-->
          <div class="d-flex flex-center flex-column mb-5">
            <!--begin::Avatar-->
            <div class="symbol symbol-100px symbol-circle mb-7">
              <!-- <img :src="getAssetPath('media/avatars/300-1.jpg')" alt="image" /> -->
              <img :src="brand.full_logo_path" alt="image" />
            </div>
            <!--end::Avatar-->

            <!--begin::Name-->
          </div>
          <!--end::Summary-->

          <!--begin::Details toggle-->
          <div class="d-flex flex-stack fs-4 py-3">
            <div
              class="fw-bold rotate collapsible"
              data-bs-toggle="collapse"
              href="#kt_customer_view_details"
              role="button"
              aria-expanded="false"
              aria-controls="kt_customer_view_details"
            >
              {{ $t('Details') }}
              <span class="ms-2 rotate-180">
                <KTIcon icon-name="down" icon-class="fs-3" />
              </span>
            </div>

            <!-- <span
                    data-bs-toggle="tooltip"
                    data-bs-trigger="hover"
                    title="Edit customer details"
                  >
                    <a
                      href="#"
                      class="btn btn-sm btn-light-primary"
                      data-bs-toggle="modal"
                      data-bs-target="#kt_modal_update_customer"
                    >
                      Edit
                    </a>
                  </span> -->
          </div>
          <!--end::Details toggle-->

          <div class="separator separator-dashed my-3"></div>

          <!--begin::Details content-->
          <div id="kt_customer_view_details" class="collapse show">
            <div class="py-5 fs-6">
              <!--begin::Badge-->
              <!-- <div class="badge badge-light-info d-inline">Premium user</div> -->
              <!--begin::Badge-->
              <!--begin::Details item-->
              <div class="fw-bold mt-5">{{ $t('arabicName') }}</div>
              <div class="text-gray-600">{{ brand.name_ar }}</div>
              <!--begin::Details item-->
              <!--begin::Details item-->
              <div class="fw-bold mt-5">{{ $t('englishName') }}</div>
              <div class="text-gray-600">{{ brand.name_en }}</div>
            </div>
          </div>
          <!--end::Details content-->
        </div>
        <!--end::Card body-->
      </div>
      <!--end::Card-->
    </div>
    <Vue3Lottie
      v-if="pending"
      :animation-data="loader"
      :height="400"
      :width="400"
    />
    <!--end::Sidebar-->
  </div>
  <!--end::Layout-->

  <NewCardModal></NewCardModal>
</template>

<script setup lang="ts">
import { getAssetPath } from "@/core/helpers/assets";
import { defineComponent, ref, onMounted, watch } from "vue";
import { useModelsStore } from "@/stores/models";
// import Dropdown3 from "@/components/dropdown/Dropdown3.vue";
import NewCardModal from "@/components/modals/forms/NewCardModal.vue";
import { Vue3Lottie } from "vue3-lottie";
import loader from "@/assets/animations/loader.json";

let props = defineProps(["id"]);
let store = useModelsStore();
let brand = ref(store.showBrand);
let pending = ref(true);

const getBrand = async () => {
  store.getItem(props.id);
  console.log(brand.value);
};

onMounted(() => {
  pending.value = true;
  getBrand();
  console.log(`stat: ${pending.value}`)
});

watch([() => store.showBrand], ([val]) => {
  brand.value = val;
  pending.value = store.pendingShow;
});
</script>
